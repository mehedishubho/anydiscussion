...
// [CITED: src/lib/post-card.ts — postUrl() / UNCATEGORIZED_SLUG, the single source of truth for the URL shape]
//
...
//
// Route path: /blog/[category]/[slug] — 260828-blog-url restructure. The category
// segment is the categories.slug value, OR the literal "uncategorized" for posts
// whose categoryId IS NULL (mirrors getPostForPublic's WHERE filter, see
// lib/queries/posts.ts). Old /blog/{slug} URLs are 308-redirected by the
// redirects-table lookup in src/proxy.ts (consumed via the backfill script).
//
 ...
 *
 * 260828-blog-url: PostLike now carries categorySlug so buildPostMetadata can
 * derive the correct canonical `/blog/{category}/{slug}` URL.
 */
export async function generateMetadata({
  params,
}: PostPageProps): Promise<Metadata> {
  const { category, slug } = await params;
  const data = await getPostForPublic(category, slug);
  if (!data) {
    return { title: "Not Found" };
  }
  // Drizzle leftJoin uses the TABLE NAME (snake_case) as the result key, not the
  // schema variable name — so postSeo → "post_seo", user → "user", categories → "categories".
  const { posts: post, post_seo: seo, user: author, categories: categoryRow } = data;
...
async function PostPageContent({ params }: PostPageProps) {
  ...
  const settings = await getSeoSettings();
  // 260828-blog-url: derive canonical from the /blog/{category}/{slug} route. When
  // the post has no category the segment falls back to "uncategorized" — mirrors
  // postUrl() in lib/post-card.ts so link generators + canonical agree.
  const canonicalRel =
    seo?.canonicalUrl ||
    `/blog/${categoryRow?.slug ?? "uncategorized"}/${post.slug}`;
        ...
        <article className="mx-auto w-full max-w-3xl">
          ...
          <div className="mb-8 flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            {/* ShareButtons — client island (D-14). Plain anchors for X/FB/LinkedIn,
                one copy-link button. SSR-safe (hrefs built after mount). 260828:
                now also takes categorySlug so the share URL points at the new
                /blog/{category}/{slug} shape. */}
            <ShareButtons
              slug={post.slug}
              title={post.title}
              categorySlug={categoryRow?.slug ?? null}
