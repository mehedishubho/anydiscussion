...
// [CITED: src/app/(admin)/dashboard/categories/CategoriesTable.tsx — identical shape]
// [CITED: src/actions/tags.ts — existing Phase 3 actions (permission-check-first)]
//
// Client component — owns the optimistic CRUD lifecycle for tags. Mirrors
// CategoriesTable. Tag rows have only name + slug (no description per Phase 3
// schema). Mutations wrap the existing createTag / updateTag / softDeleteTag.
import { useState } from "react";
import { useForm } from "react-hook-form";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { createTag, updateTag, softDeleteTag } from "@/actions/tags";
...
type EditingState = { mode: "new" } | { mode: "edit"; row: TagRow } | null;

export default function TagsTable({ initialRows }: TagsTableProps) {
  ...
  const { data: rows = initialRows } = useQuery({
    queryKey,
    queryFn: async () => {
      const { listTags } = await import("@/actions/tags");
      return (await listTags()) as TagRow[];
    },
  ...
  const createMutation = useMutation({
    mutationFn: (values: TagFormValues) =>
      createTag({ name: values.name, slug: values.slug }),
    onMutate: async (values) => {
      await queryClient.cancelQueries({ queryKey });
      const previous = queryClient.getQueryData<TagRow[]>(queryKey);
      const optimisticRow: TagRow = {
    ...
    onError: (err, _values, context) => {
      ...
      setErrorMsg(err instanceof Error ? err.message : "Failed to create tag");
    },
    onSettled: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: number; values: TagFormValues }) =>
      updateTag(id, { name: values.name, slug: values.slug }),
    onMutate: async ({ id, values }) => {
      ...
      queryClient.setQueryData<TagRow[]>(queryKey, (old = []) =>
        old.map((row) =>
          row.id === id
            ? { ...row, name: values.name, slug: values.slug }
            : row,
        ),
      );
      return { previous };
    ...
    onError: (err, _vars, context) => {
      ...
      setErrorMsg(err instanceof Error ? err.message : "Failed to update tag");
    },
    onSettled: () => {
      void queryClient.invalidateQueries({ queryKey });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => softDeleteTag(id),
    onMutate: async (id) => {
      await queryClient.cancelQueries({ queryKey });
      const previous = queryClient.getQueryData<TagRow[]>(queryKey);
      queryClient.setQueryData<TagRow[]>(queryKey, (old = []) =>
  ...
  const handleDelete = (row: TagRow) => {
    const ok = window.confirm(
      `Delete tag "${row.name}"? This is a soft-delete (D-08 — restorable via DB).`,
    );
    if (!ok) return;
    ...
    <>
      ...
      {rows.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center text-sm text-gray-500 dark:border-gray-700">
          No tags yet. Click <span className="font-medium">+ New Tag</span> to create your first tag.
        </div>
      ) : (
...
interface TagFormModalProps {
  ...
  onUpdate: (id: number, values: TagFormValues) => void;
}
...
function TagFormModal({
  ...
  const onValid = (values: TagFormValues) => {
