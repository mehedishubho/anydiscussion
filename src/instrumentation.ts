...
export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    const {
      ...
      seedPages,
      seedSeoSettings,
      seedPublicFrontendSettings,
    } = await import("@/lib/storage/seed");
    ...
    // Plan 06-01 — seed the public-frontend settings keys (contact recipient,
    // analytics script/ID, footer socials) so they exist before any public
    // route reads them (D-07/D-17).
    await seedPublicFrontendSettings();
