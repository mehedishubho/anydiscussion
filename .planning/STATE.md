...
last_updated: "2026-08-26T19:08:53.878Z"
...
**Current focus:** Phase 7 — performance-deploy

## Current Position

Phase: 7 (performance-deploy) — EXECUTING
Plan: 1 of 7
...
| 260827-869 | Dashboard menu cleanup — Components/Calendar demo nav removed with 20 orphaned routes/components ((ui-elements) showcase, calendar page, ModalExample/video/images demos, ComponentCard; owner request supersedes 04-CONTEXT D-02); sidebar now Menu + single flat Settings section (Storage/SEO/Backup/Newsletter admin-gated + Profile all-roles, D-09); header identity synced to session user (AuthGate → AdminShell → AppHeader → UserDropdown prop chain, next/image avatar with brand-toned initials fallback, router.refresh on profile save); self-service changeOwnPassword action + Change password card (shared Zod schema, digest-mapped errors, headers forwarded per 260824-qtu, revokeOtherSessions:true); TDD RED+GREEN, 676/676 vitest, tsc clean | 2026-08-27 | 0521515 | [260827-869-dashboard-menu-cleanup-remove-unused-com](./quick/260827-869-dashboard-menu-cleanup-remove-unused-com/) |
| 260827-se8 | Dashboard functional gaps — notifications data layer (notifications table + notifyUsers awaited fan-out + 3 session-scoped actions + newsletter-subscribe hook; submit-for-review→editors/admins, publish/return→author, new subscriber→admins) with functional header bell (TanStack Query island, live unread badge, mark-read-on-open); globalSearch action + GlobalSearch header island (grouped Posts/Users/Categories/Tags live results, role-safe author scoping, click→edit); URL-driven list mechanics (searchParams → list-filters helpers → Zod after permission gate; q/status/category/author + 20-row pagination + site Pagination preserving filters) on posts, users, categories, media; returnForRevision workflow (Return button + notify hooks, fills pending_review→draft gap); storage form shows all 4 provider sections simultaneously with movable Active badge (backend already 4-provider, D-25 redaction kept); listMedia opts + listCategories(opts?) paginated-only-when-asked back-compat; TDD 16 commits, 764/764 vitest (+88), build cold-clean, tsc 8 pre-existing scaffold baseline (identical before/after, table in deferred-items.md) | 2026-08-28 | c4def3c | [260827-se8-dashboard-functional-gaps-make-dashboard](./quick/260827-se8-dashboard-functional-gaps-make-dashboard/) |
| 260828-g2h | Fix 260827-se8 UAT regression — "No QueryClient set" SSR error: AdminShell rendered AppHeader (GlobalSearch + NotificationDropdown TanStack islands) OUTSIDE QueryProvider, which wrapped only {children}; provider hoisted to wrap header + page content (import site unchanged → D-28 (admin)-only bundle isolation intact, grep-verified single importer); new AdminShell.test.tsx pins the regression (3 tests: header islands render without an outer provider, useQueryClient probe inside {children}, no-throw shell render) with test-only mocks for next/navigation (null pathname in jsdom) + ten @/icons SVG modules; stale "wraps {children}" D-28 comments refreshed in AdminShell + QueryProvider (comment-only); RED proven pre-fix, 767/767 vitest, tsc at 8-error scaffold baseline | 2026-08-28 | 077a67a | [260828-g2h-fix-queryclient-ssr-error-header](./quick/260828-g2h-fix-queryclient-ssr-error-header/) |
| 260828-gyt | Posts page UAT triage — three issues, one root cause (no unpublish path existed despite TRANSITIONS legalizing published→draft for all roles): T1 action layer (listPosts rows now carry authorName/publishedAt/previewToken via author join; unpublishPost = assertOwnsPost-first → transitionPost published→draft → revalidatePublicPostSurfaces, no notify/token rotation; setSchedule status-aware — past-date-on-published rejects SCHEDULE_IN_PAST before any write, future-date writes date THEN funnels published→draft returning {ok, unpublished}, scheduled = draft + future publishedAt per A6); T2 list UI (Author column, per-row View link /blog/{slug} published or /preview/{token} drafts both _blank, blue Scheduled badge for draft + future publishedAt, editor/admin Unpublish row button in PostRowActions); T3 PostForm ("Save" label on published posts / "Save draft" otherwise; Unpublish button flipping form to draft mode; router.push to /dashboard/posts/{id}/edit after NEW-post save only via initialId==null guard; SchedulePicker toast "Post unpublished — scheduled for {date}" when unpublished); TDD 6 commits, 794/794 vitest, tsc at 8-error scaffold baseline | 2026-08-28 | 55596ac | [260828-gyt-posts-author-view-unpublish-schedule](./quick/260828-gyt-posts-author-view-unpublish-schedule/) |
