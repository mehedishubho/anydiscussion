...
- **Global scope**: `D:/Devsroom-Work/anydiscussion/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Devsroom-Work/anydiscussion/.claude/notes/`)
...
2. Glob `D:/Devsroom-Work/anydiscussion/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Devsroom-Work/anydiscussion/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Devsroom-Work/anydiscussion/.claude/notes/` — works in any directory
